...
| `UpdateEventHandler` | `UpdateEventCommand` | `UpdateEventCommandValidator` — rules: same field rules as `CreateEventCommandValidator` plus `Version` GreaterThan(0) | Updates all mutable fields; regenerates slug from new title; sets `UpdatedAt = UtcNow` before saving (must be explicit — EF Core does not auto-set `UpdatedAt` without an interceptor); calls `ICacheInvalidator` if the event is currently `Published = true` |
