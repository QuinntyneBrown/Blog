...
- **Caching**: The public GET response is served with `Cache-Control: public, max-age=60, stale-while-revalidate=600` (L2-071.3) **only when about content exists**. When `GET /api/about` returns a `null` body (no content has been authored yet), the response must be served with `Cache-Control: no-store` so that reverse proxies and CDNs do not cache the absence of content. If the null response were cached with `public, max-age=60`, a CDN edge would serve "no content" for up to 60 seconds after the author's first save — and the `ICacheInvalidator` call issued on that first save may not reach all CDN edges, leaving the empty-state response stranded in edge caches. By returning `no-store` for the null case, the first real save is visible immediately without relying on cache invalidation. The `ICacheInvalidator` is called on a successful PUT or restore to bust the cached response. `GET /api/about` supports conditional requests when content exists (populated state):
  - **`Last-Modified`**: Set to `AboutContent.UpdatedAt`, truncated to whole seconds per RFC 7232 §2.2.
  - **`ETag`**: Weak validator `W/"about:{Version}"` (e.g. `W/"about:5"`), following the Feature 07 `ETagGenerator` pattern. The version-based ETag is stable, cheap to compute (no response-body hashing), and changes on every upsert or restore.
  - **Conditional evaluation**: On requests with `If-None-Match` or `If-Modified-Since`, the handler checks whether the ETag matches or `UpdatedAt` ≤ the client's timestamp. If either condition holds, it returns **304 Not Modified** with no body. The check must occur **after** loading the singleton row but **before** serialising the response, so 304 responses avoid serialisation and Markdown-rendering overhead.
  - **Empty state**: When no about content exists (`GetCurrentAsync()` returns null), the response is `200 + null` with `Cache-Control: no-store` and **no** `ETag` or `Last-Modified` headers — there is no version metadata to derive a validator from, and the empty state must not be cached. Conditional requests against the empty state always receive 200 (not 304).
  - **Integration with `Cache-Control`**: The `Cache-Control: public, max-age=60, stale-while-revalidate=600` policy is unchanged for the populated state. Within `max-age`, clients serve from cache. After expiry, clients revalidate using conditional headers — a 304 avoids transferring the full body. `ICacheInvalidator` still purges the server-side cache on upsert/restore; the conditional headers complement active invalidation.
  - **Razor Page**: The public `/about` Razor Page should emit matching `Last-Modified` and `ETag` headers for browser conditional requests. The middleware must evaluate conditional headers before rendering the view.
...
- **First-insert concurrency**: `UpsertAboutContentHandler` performs a read-then-write (check for existing row → insert if null, update if found). Two simultaneous first-ever saves will both read null and both attempt insert with the same well-known PK (`AboutContent.WellKnownId`). The PK uniqueness constraint rejects the second insert — this is the database-level singleton enforcement in action. The handler must catch the resulting `DbUpdateException` and retry as an update. See **§7 — First-insert concurrency** for the full rationale.
...
- **Dependencies**: `AboutContentRepository`, `DigitalAssetRepository` (to verify historical profile image still exists on restore), `IMarkdownConverter`, `ICacheInvalidator`
- **Steps**: (1) Load history record by `historyId`; return 404 if not found. (2) Verify `AboutContentId` on the loaded history record matches the singleton's `AboutContentId`; return 404 if they do not match — this prevents cross-resource access and is consistent with the "not a valid restore target" semantics (in a correctly operating system this mismatch cannot occur because there is only one `AboutContent` row, but it must be handled defensively against data corruption). (3) Load the current live row; if the supplied `currentVersion` does not match the stored `Version`, return 409 (prevents a restore from silently overwriting concurrent edits). (4) Snapshot the current live row into `AboutContentHistory`; the handler must set `ArchivedAt = UtcNow` on the new history entity — EF Core does not auto-populate this field and there is no DB default for the column; leaving it unset would persist `DateTime.MinValue` rather than the actual snapshot time. (5) Overwrite live row fields from the history snapshot: copy `Heading` and `Body` unconditionally. For `ProfileImageId`: if the history snapshot's `ProfileImageId` is not null, check whether the referenced asset still exists by calling `DigitalAssetRepository.ExistsAsync(historyRecord.ProfileImageId)`. If the asset exists, copy `ProfileImageId` to the live row. If the asset has been deleted (or `ProfileImageId` is null in the snapshot), set `ProfileImageId = null` on the live row — this avoids a FK violation against the `ON DELETE SET NULL` constraint on the live `AboutContent` table and ensures the restore always succeeds. Set a `profileImageRestored` flag to `false` when the image could not be restored so the response can inform the author. Re-render `BodyHtml` by calling `IMarkdownConverter.Convert(historyRecord.Body)` rather than copying the snapshot's `BodyHtml` directly — this ensures the restored HTML is produced under the current sanitizer allow-list rather than stale rules that may have been in effect when the snapshot was originally saved. Set `UpdatedAt = UtcNow`; increment `Version`. `CreatedAt` and `AboutContentId` must NOT be copied from the snapshot — `CreatedAt` is the singleton's original creation date and must remain stable; `AboutContentId` is the PK and must never change. (6) Call `ICacheInvalidator.InvalidateAsync("/about")`. Steps 4 and 5 must execute within a single DB transaction so that a failure between them cannot produce an orphaned history snapshot with no corresponding live-row update.
...
- **Conditional response**: When returning a populated response, the handler must set `ETag` (`W/"about:{Version}"`) and `Last-Modified` (`UpdatedAt`) response headers, and evaluate `If-None-Match` / `If-Modified-Since` request headers to return 304 when the content has not changed (see §3.1 Caching). The conditional check is performed after loading the row but before projecting to `PublicAboutContentDto`.
...
- **Key methods**:
  - `GetCurrentAsync()` — calls `FindAsync(AboutContent.WellKnownId)` (or equivalently `SingleOrDefaultAsync(x => x.AboutContentId == AboutContent.WellKnownId)`) with a **LEFT JOIN** to the `DigitalAssets` table on `ProfileImageId`. Using the well-known PK for lookup is deterministic — unlike `FirstOrDefaultAsync()` over an unconstrained table, it cannot return non-deterministic results if duplicate rows somehow exist (which the PK constraint prevents). The LEFT JOIN is required because `ProfileImageId` is nullable — when no profile image is set (or when an asset has been deleted and `ON DELETE SET NULL` has nulled the reference), the about content row must still be returned. With the FK constraint in place, a non-null `ProfileImageId` always references an existing asset — dangling references are impossible by construction.
  - `AddAsync(entity)` — insert
  - `UpdateAsync(entity)` — update (caller increments version)
- **Notes**: The repository uses the well-known `AboutContent.WellKnownId` constant for all lookups. No arbitrary-ID-based lookup is needed — there is always exactly zero or one row, enforced by the PK constraint. No `GetAllAsync` or pagination.
...
| Field | Type | Notes |
|-------|------|-------|
| `AboutContentId` | `Guid` | PK; **well-known constant** `00000000-0000-0000-0000-000000000001`. The singleton invariant is enforced at the database level by always using this fixed PK — the primary key uniqueness constraint prevents a second row from being inserted. The value is defined as a static constant `AboutContent.WellKnownId` in the domain layer. |
...
| `ProfileImageId` | `Guid?` | FK → `DigitalAssets.DigitalAssetId`; nullable; **`ON DELETE SET NULL`** — when a digital asset is deleted, SQL Server automatically sets this column to `NULL` rather than blocking the delete or leaving a dangling reference. EF Core configuration: `.HasOne<DigitalAsset>().WithMany().HasForeignKey(x => x.ProfileImageId).OnDelete(DeleteBehavior.SetNull)`. |
...
**FK strategy — ON DELETE SET NULL**: The `ProfileImageId` FK uses `ON DELETE SET NULL` rather than `ON DELETE RESTRICT` (which would block asset deletion when referenced by the about page) or `ON DELETE CASCADE` (which would delete the about content when an asset is deleted — clearly wrong for a singleton). `SET NULL` is the correct choice: deleting an asset gracefully removes the profile image from the about page without blocking the delete or corrupting the about content. After `SET NULL`, the public page renders without a profile image until the author sets a new one.

**Design note — singleton enforcement**: The table is constrained to exactly one row by using a well-known fixed primary key (`00000000-0000-0000-0000-000000000001`). The PK uniqueness constraint makes it impossible to insert a second row — any attempt receives a primary key violation from the database engine. `UpsertAboutContentHandler` calls `GetCurrentAsync()` — if null, it inserts with `AboutContentId = AboutContent.WellKnownId`; otherwise it copies the current row to `AboutContentHistory` then updates within a single DB transaction. No additional unique constraints or discriminator columns are needed — the fixed PK is sufficient.
...
| `ProfileImageId` | `Guid?` | Snapshot value — **no FK constraint**. Storing this as a live FK to `DigitalAssets` would cause `ON DELETE RESTRICT` to block asset deletion whenever a history snapshot references that asset. Since history rows are immutable records of past state, the `ProfileImageId` value here is informational only; the corresponding asset may no longer exist. On restore, the handler validates the asset still exists before copying the ID to the live row — if it does not, the live row's `ProfileImageId` is set to `null` (see §3.3 step 5). |
...
|--------|------|------|---------------|---------|--------|
| `GET` | `/api/about` | None | — | 200 + `PublicAboutContentDto?` / 304 | — |
...
| `GET` | `/api/about/history` | Bearer token | `?page&pageSize` (default page=1, pageSize=20, max pageSize=50) | 200 + `PagedResponse<AboutContentHistoryDto>` — returns an empty `items` array (`items: [], totalCount: 0`) when no history exists (i.e. the about content has never been saved or has been saved only once with no prior snapshot); never returns 404 for an empty history | 401 |
...
**Restore response enrichment**: When `PUT /api/about/restore/{historyId}` returns 200, the `AboutContentDto` response includes an additional field `profileImageRestored` (`bool`). This is `true` when the historical profile image was successfully restored, and `false` when the image asset no longer exists and `ProfileImageId` was set to `null`. The back-office UI should display a notification when `profileImageRestored` is `false` (e.g. "The profile image from this revision is no longer available. Please upload a new image."). For non-restore responses (GET, PUT upsert), this field is omitted or null.
...
- **Input length validation**: `UpsertAboutContentHandler` enforces domain-appropriate size limits server-side: `Heading` ≤ 256 chars; `Body` ≤ 50,000 chars (approximately 10,000 words of Markdown — generous for biography content but bounded at the application layer rather than relying solely on the Kestrel 1 MB infrastructure limit). Requests exceeding these limits are rejected with 400 before any persistence or Markdown rendering occurs. The `Body` limit is enforced on the **upsert path only** — `RestoreAboutContentHandler` does not re-validate body length because history rows were accepted under the rules in effect at the time they were saved, and rejecting a restore due to a retrospectively tightened limit would make historical revisions unrestorable. The `Body` column remains `nvarchar(max)` in the schema; the 50,000-character limit is an application-layer policy, not a database constraint, so it can be adjusted without a schema migration.
...
- **First-insert concurrency**: `UpsertAboutContentHandler` performs a read-then-write (check for existing row → insert if null, update if found). Two simultaneous first-ever saves will both read null and both attempt insert with the same well-known PK (`AboutContent.WellKnownId`). The **primary key uniqueness constraint** rejects the second insert — this is the database-level singleton enforcement in action. The handler must catch the resulting `DbUpdateException` and retry the operation as an update. This catch-and-retry approach is preferred over a serializable DB transaction because it avoids elevated isolation level overhead on every save and naturally leverages the PK constraint as the singleton guard. After the retry, the losing writer's update applies optimistic concurrency checks normally (version comparison), so no data is silently lost.
- **`profileImageId` validation and FK integrity**: `UpsertAboutContentHandler` validates `profileImageId` (calls `DigitalAssetRepository` to verify the asset exists and is owned by the requesting user) before the EF Core save. The FK constraint on `AboutContent.ProfileImageId` (`ON DELETE SET NULL`) ensures referential integrity at the database level — a non-null `ProfileImageId` always references a valid `DigitalAsset` row. If the asset is deleted between the validation step and the `SaveChangesAsync` call (a sub-millisecond TOCTOU window), the FK constraint rejects the save with a `DbUpdateException` (foreign key violation). The handler must catch this exception and return 409 Conflict with a message indicating the profile image no longer exists — the client should retry without a profile image or select a different one. This is a strict improvement over the previous design, which accepted dangling references as a normal-state outcome. The `AboutContentHistory` table intentionally has **no FK** on its `ProfileImageId` (§4.2) — history rows are immutable snapshots and the referenced asset may have been deleted after the snapshot was taken.
