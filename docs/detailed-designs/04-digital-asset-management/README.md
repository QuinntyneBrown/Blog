...
|---|----------|--------|--------|
| 1 | ~~Should we use local filesystem or cloud blob storage?~~ **Resolved: Local filesystem for v1.** Cheapest and simplest. The `AssetStorage` abstraction (`IAssetStorage`) allows swapping to Azure Blob Storage later without application changes. | Deployment architecture, cost, operational complexity | Resolved |
...
| 6 | ~~Should asset deletion be supported?~~ **Resolved: Hard delete with orphan protection.** Deletion is allowed only for assets not referenced by any article's `FeaturedImageId`. The API returns 409 Conflict if the asset is in use. Inline `<img>` references in article Markdown bodies are the author's responsibility to update before deleting. | Data integrity, storage management | Resolved |
