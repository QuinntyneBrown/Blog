...
| 3 | ~~Should account lockout be implemented after N failed attempts, or is rate limiting sufficient?~~ **Resolved: Rate limiting only.** The dual rate limit (10/min per IP + 5/15min per email) provides sufficient protection. Account lockout introduces a denial-of-service vector where attackers can intentionally lock out legitimate users. | Security vs. usability tradeoff | Resolved |
| 4 | ~~Will the External Identity Provider (OAuth/OIDC) integration be needed in a near-term release?~~ **Resolved: Deferred.** Not needed for v1 personal blog. The JWT-based auth abstraction is sufficient. OAuth/OIDC can be added later behind the existing `IAuthService` interface without breaking changes. | Architecture extensibility | Resolved |
