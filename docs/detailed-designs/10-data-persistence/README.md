...
| 2 | ~~What connection pooling strategy should be used?~~ **Resolved: EF Core default pooling.** `DbContext` pooling via `AddDbContextPool<BlogDbContext>()` is sufficient for a personal blog's load. No external pooler needed. Cheapest and simplest. | Performance under load, resource utilization | Resolved |
| 3 | ~~Should read replicas be introduced?~~ **Resolved: No.** Single-instance Azure SQL Database is sufficient for a personal blog. Read replicas add cost and consistency complexity. Can be added later if load warrants it. | Scalability, complexity, consistency model | Resolved |
| 4 | ~~Should the migration runner be an IHostedService or a separate CLI tool?~~ **Resolved: IHostedService on startup.** Simplest deployment model — app self-migrates before accepting traffic. For a single-instance personal blog, this avoids a separate CI/CD migration step. Production can switch to CLI-based migration later if zero-downtime deployments are needed. | Deployment strategy, zero-downtime deployments | Resolved |
| 5 | ~~What is the data retention policy for soft-deleted records?~~ **Resolved: N/A.** Soft delete was not adopted (see Feature 02). Permanent deletion per L2-004. | Storage growth, GDPR compliance | Resolved |
| 6 | ~~Should the `Body` column use a full-text search index?~~ **Resolved: SQL Server `LIKE` for v1.** Simple `LIKE '%term%'` search on title and abstract is sufficient for a personal blog with a small article count. SQL Server full-text indexing (`CONTAINS`/`FREETEXT`) can be enabled later with no schema changes if search performance degrades. Cheapest — no additional infrastructure. | Search performance, infrastructure complexity | Resolved |
