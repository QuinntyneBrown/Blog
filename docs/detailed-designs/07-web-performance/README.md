...
|---|---|---|---|
| OQ-1 | ~~**SSG vs SSR**~~ **Resolved: SSR only.** Server-side rendering with in-memory response caching is sufficient. SSG adds build pipeline complexity and a publish step for minimal gain on a personal blog with response caching already achieving sub-200ms TTFB. Simplest and most maintainable. | High — could simplify caching and improve P99 TTFB | Resolved |
| OQ-2 | ~~**CDN strategy**~~ **Resolved: Deferred.** No CDN for v1. Direct origin serving is sufficient for a personal blog. Azure Front Door can be added later without application changes. Cheapest option. | Medium — depends on target audience geography | Resolved |
...
| OQ-5 | ~~**RUM analytics endpoint**~~ **Resolved: Application Insights.** Core Web Vitals data collected via `web-vitals` library is sent to Application Insights (already provisioned for logging, Feature 09). No additional infrastructure or cost. Custom events tracked via the Application Insights JS SDK snippet. | Low — does not affect user-facing performance | Resolved |
