...
| 2 | ~~What alerting strategy should be used?~~ **Resolved: Azure Monitor Alerts.** Native integration with Application Insights. Alert rules for: 5xx error rate > 5% over 5 minutes, health check failures, TTFB P95 > 200ms. Alerts delivered via email (cheapest). PagerDuty/Opsgenie can be added later as an action group. | Determines integration with PagerDuty, Opsgenie, or similar. | Resolved |
...
| 4 | ~~What is the log retention policy for production?~~ **Resolved: 30 days.** Application Insights default retention. Cheapest tier. Sufficient for a personal blog's operational needs. Can be extended later if needed. | Affects storage costs and compliance. | Resolved |
| 5 | ~~Should OpenTelemetry be adopted from the start?~~ **Resolved: Deferred.** Single-service deployment does not benefit from distributed tracing. Serilog + Application Insights provides sufficient observability. OpenTelemetry can be layered in later via the `Azure.Monitor.OpenTelemetry.Exporter` package if the architecture evolves to multi-service. | Affects library choices and instrumentation approach. | Resolved |
