...
- **RFC 8058 endpoint security**: The `POST /api/newsletter-subscriptions/unsubscribe` endpoint validates that the body matches `List-Unsubscribe=One-Click` exactly. This prevents the endpoint from being repurposed as a generic unsubscribe API by non-RFC-8058 callers. The token in the query parameter is validated via HMAC (§3.10) — forging a valid token requires the server secret. The endpoint does not accept `application/json` — only `application/x-www-form-urlencoded` — reducing the attack surface. The always-200 response prevents automated systems from enumerating valid tokens by observing response codes.
