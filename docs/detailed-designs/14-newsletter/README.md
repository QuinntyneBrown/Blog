...
| `Payload` | `nvarchar(max)` | JSON-serialised message body. For `NewsletterDelivery`: `{ newsletterId, subscriberId, email, unsubscribeToken }`. For `ConfirmationEmail`: `{ subscriberId, email, confirmUrl }` |
...
**Idempotency on replay**: For `NewsletterDelivery` messages, replay is safe because the Service Bus consumer's `NewsletterSendLog` check (§3.8) prevents duplicate emails. For `ConfirmationEmail` messages, replay is safe because sending a second confirmation email is harmless — the subscriber clicks the same link; if the subscriber has already confirmed, the token will have been cleared (`ConfirmationToken = null`) and `ConfirmSubscriptionHandler` returns 422 (token not found), which the subscriber can safely ignore.
...
- **Payload**: `{ subscriberId, email, confirmUrl }` where `confirmUrl` is the full HTTPS URL containing the confirmation token.
...
- **Token generation and storage**: Confirmation and unsubscribe tokens are generated with `RandomNumberGenerator.GetBytes(32)` (256 bits from a CSPRNG), hex-encoded to a 64-character string. `Guid.NewGuid()` must NOT be used — version-4 GUIDs use a non-cryptographic pseudo-random number generator on some runtimes, making the token space predictable. **Raw tokens are never stored in the database.** Only the SHA-256 hash (hex-encoded, 64 chars) is persisted in `ConfirmationTokenHash` / `UnsubscribeTokenHash`. The raw token is included in the email link (for confirmation) or URL (for unsubscribe) and in outbox message payloads, but never in a durable column. This ensures a database read leak does not expose usable bearer tokens — an attacker who obtains the hash cannot reverse it to produce a valid confirmation or unsubscribe URL. SHA-256 is a one-way hash; the 256-bit input entropy ensures brute-force pre-image attacks are computationally infeasible. Lookup is performed by hashing the presented token and querying against the stored hash. Raw tokens must never appear in logs, telemetry, dead-letter error messages, or admin replay tooling — only the hash or a truncated prefix (first 8 hex chars) may be logged for correlation.
...
4. **Re-subscribe flow**: ~~New record or reactivate existing?~~ **Resolved**: The existing record is reactivated. `SubscribeHandler` checks for an existing row with the same email; if found with `IsActive = false`, it sets `IsActive = true`, generates a **new** CSPRNG confirmation token, computes its SHA-256 hash, and stores the hash in `ConfirmationTokenHash` (overwriting any stale hash). The raw token is embedded in the `confirmUrl` of the outbox payload. Resets `TokenExpiresAt` to 48 hours from now, updates `ResubscribedAt`, and **resets `Confirmed = false`**. A `ConfirmationEmail` outbox row is written in the same transaction, and the subscriber must re-confirm before receiving newsletters. Re-confirmation is required on resubscribe regardless of whether the subscriber was previously confirmed — a gap in time between unsubscribe and resubscribe means the email address ownership should be re-verified; additionally, since `ConfirmationTokenHash` is overwritten with the hash of a fresh token, the subscriber must complete the new opt-in flow. No duplicate records are created. The old token must not be reused — it may have already expired or been observed by an attacker who intercepted the original confirmation email.
