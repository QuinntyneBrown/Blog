...
|---------|---------|-----------|--------|
| `CreateNewsletterHandler` | `CreateNewsletterCommand` | `CreateNewsletterCommandValidator` — rules: `Subject` NotEmpty, MaximumLength(512); `Body` NotEmpty | Persists newsletter with `status=Draft` |
...
| `UnsubscribeHandler` | `UnsubscribeCommand` | — | Sets `isActive=false` by unsubscribe token (L2-056). Always returns 204 regardless of whether the token exists, is already inactive, or was never issued — the endpoint is fully idempotent and must not reveal token validity. `GetSubscriberByUnsubscribeTokenAsync` returns `null` on a miss; the handler treats null as a no-op and returns 204 (not 404), preventing token enumeration by observers who can distinguish 204 from 404. |

### 3.4 NewsletterRepository

- **Responsibility**: Data access for `Newsletter`, `NewsletterSubscriber`, and `NewsletterSendLog` entities.
- **Key methods**: `GetByIdAsync`, `GetAllAsync(page, pageSize, status?)`, `GetBySlugAsync(slug)`, `SlugExistsAsync(slug)`, `StreamConfirmedSubscriberIdsAsync()`, `GetSubscribersAsync(page, pageSize)`, `GetSubscriberByEmailAsync(email)`, `GetSubscriberByConfirmationTokenAsync(token)`, `GetSubscriberByUnsubscribeTokenAsync(token)`, `AddSubscriberAsync(entity)`, `UpdateSubscriberAsync(entity)`
...
- **Notes**: List queries project only `Subject`, `Slug`, `Status`, `DateSent`, `CreatedAt` — `Body`/`BodyHtml` excluded for performance (same pattern as `ArticleRepository`). `StreamConfirmedSubscriberIdsAsync()` uses `IAsyncEnumerable<Guid>` to avoid loading all subscriber IDs into memory before enqueuing to Service Bus. Subscriber lookup methods (`GetSubscriberByConfirmationTokenAsync`, `GetSubscriberByUnsubscribeTokenAsync`) return `null` when the token is not found; callers are responsible for returning the appropriate 422/204 response. `GetSubscriberByEmailAsync` returns `null` when no matching row exists; `SubscribeHandler` uses this to detect the upsert path. `AddSubscriberAsync` inserts a new subscriber row. `UpdateSubscriberAsync` saves changes to an existing subscriber row (confirm, unsubscribe, resubscribe paths).
