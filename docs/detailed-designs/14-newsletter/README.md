...
- **Interfaces**:
  ...
  - `POST /api/newsletter-subscriptions/unsubscribe` — RFC 8058 one-click unsubscribe (anonymous, called by mailbox providers); accepts `application/x-www-form-urlencoded` body with `List-Unsubscribe=One-Click`; the unsubscribe token is passed as a query parameter `?token=...`; returns 200; idempotent — always returns 200 regardless of token validity to prevent enumeration by automated systems. This endpoint is separate from `DELETE /api/newsletter-subscriptions/{token}` which serves human-initiated unsubscribe from the browser UI.
...
| `OneClickUnsubscribeHandler` | `OneClickUnsubscribeCommand` | `OneClickUnsubscribeCommandValidator` | RFC 8058 one-click unsubscribe handler. Receives the unsubscribe token from the `token` query parameter. Validates that the request body is `application/x-www-form-urlencoded` and contains `List-Unsubscribe=One-Click` (per RFC 8058 §3). Calls `IUnsubscribeTokenService.ValidateAndExtractSubscriberId(token)` (§3.10) to verify the HMAC signature and extract the `subscriberId`. On valid HMAC, looks up the subscriber by `subscriberId` (PK), sets `IsActive = false`, and clears `ConfirmationTokenHash = null` and `TokenExpiresAt = null`. Always returns 200 regardless of whether the HMAC is valid, the subscriber exists, or is already inactive — RFC 8058 requires the endpoint to return a success response to prevent mailbox providers from retrying or flagging the sender. The 200 (not 204) response is intentional: some mailbox providers interpret non-200 success codes as errors. `OneClickUnsubscribeCommandValidator` rules: `Token` must be non-empty (NotEmpty); `Body` must equal `List-Unsubscribe=One-Click` (exact match, case-sensitive). Requests failing these rules are rejected with 200 (not 400) to avoid leaking validation semantics to automated callers. |
...
- **`NewsletterSendLog` methods**: `SendLogExistsAsync(newsletterId, recipientIdempotencyKey)` — returns `bool`; called by `NewsletterEmailDispatchService` before sending to enforce idempotency (the caller computes the key via `SHA256Hex($"{newsletterId}:{subscriberId}")` and passes it in). `AddSendLogAsync(entity)` — inserts a `NewsletterSendLog` row atomically with message completion; the caller must set `RecipientIdempotencyKey` on the entity before calling. These methods belong on `NewsletterRepository` (same bounded context) rather than a separate repository class — the log table is an implementation detail of the send pipeline and not queried from any other component.
...
| `RecipientIdempotencyKey` | `nvarchar(64)` | SHA-256 hash (hex-encoded) of `{NewsletterId}:{SubscriberId}`, computed by the Service Bus consumer at send time. Serves as the idempotency key — never nulled on GDPR erasure because it contains no reversible PII (the subscriberId cannot be recovered from the hash). Replaces `SubscriberId` as the idempotency lookup column. |
| `SentAt` | `datetime2` | UTC timestamp when the email was confirmed sent by the Service Bus consumer |

**Unique constraint**: `UQ_NewsletterSendLog_Newsletter_Recipient` on `(NewsletterId, RecipientIdempotencyKey)` — enforces idempotency. Unlike the previous `(NewsletterId, SubscriberId)` filtered index, this constraint does not break after GDPR erasure: `RecipientIdempotencyKey` is a one-way hash that is never nulled and contains no PII. The index is **not filtered** — `RecipientIdempotencyKey` is always non-null, so no `WHERE` clause is needed. EF Core configuration: `.HasIndex(x => new { x.NewsletterId, x.RecipientIdempotencyKey }).IsUnique()`. The old filtered index on `(NewsletterId, SubscriberId)` is removed — `SubscriberId` retains its `ON DELETE SET NULL` FK for audit purposes but no longer participates in idempotency enforcement.

**Recommended index**: The unique constraint index on `(NewsletterId, RecipientIdempotencyKey)` also serves as the lookup index for the idempotency check. A separate index on `SentAt` is only needed if the retention purge job filters by `SentAt` alone rather than using a batch-delete approach; for a 90-day purge a table scan on a small log table is acceptable.
...
- Emails include RFC 8058 one-click unsubscribe headers: `List-Unsubscribe: <https://{host}/api/newsletter-subscriptions/unsubscribe?token={unsubscribeToken}>` and `List-Unsubscribe-Post: List-Unsubscribe=One-Click`. The unsubscribe token is HMAC-derived from `subscriberId` (§3.10) and is not stored in the database. When a mailbox provider (Gmail, Yahoo, etc.) processes a one-click unsubscribe, it sends a POST request to the `List-Unsubscribe` URL with `Content-Type: application/x-www-form-urlencoded` and body `List-Unsubscribe=One-Click`. The `OneClickUnsubscribeHandler` processes this request. The human-facing unsubscribe flow (clicking the unsubscribe link in the email footer, which opens a browser confirmation page that issues `DELETE /api/newsletter-subscriptions/{token}`) remains separate and is handled by `UnsubscribeHandler`.
...
| `POST` | `/api/newsletter-subscriptions/unsubscribe` | Query: `?token`; Body: `List-Unsubscribe=One-Click` (form-urlencoded) | 200 | — |

**RFC 8058 one-click unsubscribe**: The `POST /api/newsletter-subscriptions/unsubscribe` endpoint implements RFC 8058 §3. It accepts `Content-Type: application/x-www-form-urlencoded` (not `application/json`) because mailbox providers send form-encoded bodies. The endpoint always returns 200 with an empty body — it must never return 4xx or 5xx to automated callers, as providers may interpret errors as delivery failures and penalise the sender's reputation. The `token` query parameter contains the HMAC-derived unsubscribe token (§3.10). This endpoint is rate-limited under the same `write-endpoints` sliding-window policy as the other subscription endpoints to prevent abuse, but the rate limit response (429) is an acceptable exception — providers retry 429 responses with backoff. The endpoint is distinct from `DELETE /api/newsletter-subscriptions/{token}` which serves human-initiated browser unsubscribe and returns 204.

### Public archive endpoints (no auth)

| Method | Path | Params | Success | Errors |
|--------|------|--------|---------|--------|
| `GET` | `/api/newsletters/archive` | `?page&pageSize` | 200 + `PagedResponse<NewsletterArchiveDto>` / 304 | — |
| `GET` | `/api/newsletters/archive/{slug}` | — | 200 + `NewsletterArchiveDetailDto` / 304 | 404 |
...
**ETag / conditional GET**: Both public archive endpoints support conditional requests for efficient revalidation.

  - **`GET /api/newsletters/archive/{slug}`**: The response includes `Last-Modified` set to the newsletter's `DateSent` timestamp (truncated to whole seconds per RFC 7232 §2.2) and a weak `ETag`: `W/"{slug}"`. Because sent newsletters are **immutable** (content, slug, and DateSent never change after send), the ETag is permanently stable — every conditional revalidation returns **304 Not Modified** once the content has been cached. This is the ideal conditional GET scenario: the detail page is cache-friendly indefinitely.
  - **`GET /api/newsletters/archive`**: The response includes `Last-Modified` set to the most recent `DateSent` across all sent newsletters (or omitted if no newsletters exist). The weak `ETag` is `W/"archive:{count}:{latestDateSentEpoch}"` where `count` is the total number of sent newsletters and `latestDateSentEpoch` is the most recent `DateSent` as Unix epoch seconds. The ETag changes only when a new newsletter is sent (count increments and/or latestDateSent advances). Pagination parameters (`page`, `pageSize`) are **not** included in the ETag — the ETag represents the underlying data version, not a specific page view. Different page requests for the same data version will share the same ETag, which is correct: the data has not changed even though the client is viewing a different slice.
  - **Integration with `Cache-Control`**: The existing `Cache-Control: public, max-age=300, stale-while-revalidate=600` policy is unchanged. During `max-age`, clients serve from cache. After expiry, conditional revalidation using `If-None-Match` / `If-Modified-Since` avoids transferring the full body when content has not changed. `ICacheInvalidator` still purges the server-side cache when a new newsletter is sent; the conditional headers complement active invalidation.
  - **Empty archive**: When no newsletters have been sent, `GET /api/newsletters/archive` returns `200` with an empty `items` array. No `ETag` or `Last-Modified` headers are emitted for the empty state — there is no version metadata to derive them from. The response uses the standard `Cache-Control` header; conditional requests against the empty state always receive 200.
  - **Razor Pages**: The public newsletter archive Razor Pages should emit matching `Last-Modified` and `ETag` headers. The middleware must evaluate conditional headers before rendering to avoid wasting render cycles on 304 responses.
...
- **Rate limiting**: The public newsletter subscription endpoints require dedicated rate-limit policies because they are anonymous — the `write-endpoints` policy (Feature 08 §3.3, 60 req/min per authenticated user) cannot apply since there is no authenticated user to partition by. Three policies are defined:
  - **`newsletter-subscribe`** — applies to `POST /api/newsletter-subscriptions`:
    - Per-IP limit: **5 requests per 10-minute sliding window**. Prevents a single source from triggering mass subscription-bombing (each request causes an outbox-dispatched confirmation email).
    - Per-normalized-email limit: **2 requests per 1-hour sliding window**. The email address from the request body is lowercased and trimmed before hashing to form the partition key (the plaintext email must never appear in rate-limiter storage or logs). This prevents an attacker from re-triggering confirmation emails for the same address faster than the 48-hour token expiry window warrants.
    - Both limits are evaluated independently — a request must pass both to proceed.
  - **`newsletter-confirm`** — applies to `POST /api/newsletter-subscriptions/confirm`:
    - Per-IP limit: **10 requests per 10-minute sliding window**. Prevents brute-force token enumeration. Although tokens are 256-bit CSPRNG values (infeasible to guess), the rate limit adds defence-in-depth and bounds the load that a single source can impose on the confirmation lookup path.
  - **`newsletter-unsubscribe`** — applies to `POST /api/newsletter-subscriptions/unsubscribe` (RFC 8058) and `DELETE /api/newsletter-subscriptions/{token}`:
    - Per-IP limit: **30 requests per 1-minute sliding window**. More permissive than subscribe/confirm because mailbox providers may batch one-click unsubscribe requests from a single IP when processing a bulk send. A 429 response to a mailbox provider is acceptable — providers retry with backoff (RFC 8058 §3) — but the limit should be high enough to avoid routine throttling during normal newsletter sends.
  - **Implementation**: All three policies use `SlidingWindowRateLimiter` from `System.Threading.RateLimiting` (same library as Feature 08). Policies are registered in `Program.cs` via `AddRateLimiter` and applied per-endpoint using `[EnableRateLimiting("policy-name")]` attributes on the controller actions. When a request is rate-limited, the middleware returns **429 Too Many Requests** with a `Retry-After` header indicating the number of seconds until the next window opens. Rate-limited requests must be logged at `Warning` level with the event name `newsletter.rate_limited` and fields `{ endpoint, partitionKey, policy }` — the partition key must be the hashed value (for email-based partitions) or the IP address, never a raw email.
